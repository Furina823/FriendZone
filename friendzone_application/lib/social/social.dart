  ...
  void initState() {
    ...
    futureUsers = fetchUsers().then((users) async {
      ...
      final friendUserIds = friends
          .where((friend) => (friend.user_id.toString() == widget.user_id &&
              friend.user_id_2.toString() != widget.user_id))
          .map((friend) => friend.user_id_2.toString())
          .toSet();
      final filteredUsers = users.where((user) {
        return friendUserIds.contains(user.user_id.toString()) ||
            user.user_id.toString() == widget.user_id;
      }).toList();
